  ...
  if (!this->isKinematic) {
    ...
    for (int i = 0; i < objects.size(); i++) {
      if (objects[i]->name != this->go->name) {
        if (this->go->pos.x <
                objects[i]->pos.x + objects[i]->width * objects[i]->scale &&
            this->go->pos.x + width * this->go->scale > objects[i]->pos.x &&
            this->go->pos.y <
                objects[i]->pos.y + objects[i]->height * objects[i]->scale &&
            this->go->pos.y + height * this->go->scale > objects[i]->pos.y) {
          if (objects[i]->components->getComponent("RIGIDBODYCOMPONENT") !=
              nullptr) {
            RigidBody *that = static_cast<RigidBody *>(
                objects[i]->components->getComponent("RIGIDBODYCOMPONENT"));
            if (!that->isKinematic) {
              ...
              float mtdX =
                  deltaX * ((this->go->width + that->go->width - deltaLength) /
                            deltaLength);
              float mtdY =
                  deltaY *
                  ((this->go->height + that->go->height - deltaLength) /
                   deltaLength);
              ...
              this->addForce(this->velX + (impulseX * im1),
...
void RigidBody::setGravity(float g) { this->gravity = g; };

void RigidBody::setMass(float mass) { this->mass = mass; };

void RigidBody::setDrag(float drag) { this->drag = drag; };

void RigidBody::setUseGravity(bool huh) { this->useGravity = huh; };

void RigidBody::setIsKinematic(bool huh) { this->isKinematic = huh; };
...
float RigidBody::getForceX() { return this->velX; }
float RigidBody::getForceY() { return this->velY; }

void RigidBody::setForceX(float x) { this->velX = x; }

void RigidBody::setForceY(float y) { this->velY = y; }

void RigidBody::setMaxForceX(float x) { this->maxVelX = x; }

void RigidBody::setMaxForceY(float y) { this->maxVelY = y; }
